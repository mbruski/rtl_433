...
- 3eb3 messages are a power reading of LL HH(-0x33) 0x44
...
Then we have two cases, messages with 414b or 565a which likely is a message type.
On 414b the two byte (little endian) power value follows. For the other type it should be the kWh probably.
...
static int ecodhome_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    if (m_type == 0x7700) {
        ...
        data = data_make(
                ...
                "power_W",      "Power",        DATA_FORMAT, "%.1f W",   DATA_DOUBLE, (double)power_w,
        ...
        int power_w = ((msg[7] - 0x33) << 8) | (msg[6]);
